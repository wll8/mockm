...
with (util) {
  describe('基本功能', () => {
    describe('显示版本号', () => {
      it(`获取版本号`, () => {
        const version = require(pkgPath('../dist/package/package.json')).version
    ...
    describe('config.dataDir', () => {
      it(`httpData`, async () => {
        assert.ok(hasFile(pkgPath(`../httpData`)))
      })
    })

    describe('config.dbJsonPath', () => {
      it(`dbJsonPath`, async () => {
        assert.ok(hasFile(pkgPath(`../httpData/db.json`)))
      })
    })
  ...
  describe('命令行', () => {
    it(`配置文件`, async () => {
      const {fullCmd: cmd} = await craeteMockmCmdInfo({
        '--config': uuid(`-`), // 故意找不到配置文件
      })
    ...
    it(`--cwd`, async () => {
      ...
      const hasFileRes = (
        hasFile(`${arg[`--cwd`]}/mm.config.js`)
        && hasFile(`${arg[`--cwd`]}/httpData`)
        && hasFile(`${arg[`--cwd`]}/apiWeb.json`)
      )
    ...
    it(`--template`, async () => {
      ...
      await sleep()
      const hasFileRes = (
        hasFile(`${arg[`--cwd`]}/mm/mm.config.js`)
        && hasFile(`${arg[`--cwd`]}/mm/httpData`)
        && hasFile(`${arg[`--cwd`]}/mm/apiWeb.json`)
      )
      assert.ok(hasFileRes)
    })
  })
